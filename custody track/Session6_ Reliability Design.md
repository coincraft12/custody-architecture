...
### 4.1.1 관측성 실습: 로그에서 한 건을 끝까지 따라가기

관측성 설명이 추상적으로 끝나지 않게 하려면, 강의에서 아래와 같은 “한 건 추적” 예시를 보여 주는 편이 좋다.

예시 로그 흐름:

```text
cid=9f3... withdrawal_id=w123 event=withdrawal.requested
cid=9f3... withdrawal_id=w123 event=policy.checked result=allow policy_decision_id=p456
cid=9f3... withdrawal_id=w123 event=approval.granted approval_bundle_id=a789
cid=9f3... withdrawal_id=w123 attempt_id=t001 nonce_key=eth:0xabc:104 event=attempt.created
cid=9f3... withdrawal_id=w123 attempt_id=t001 tx_hash=0xaaa event=broadcast.success
cid=9f3... withdrawal_id=w123 attempt_id=t001 tx_hash=0xaaa event=tracker.included
cid=9f3... withdrawal_id=w123 event=ledger.settle
```

강의 포인트:

1. 요청 단위 추적은 `correlation_id`로 본다.
2. 업무 단위 전이는 `withdrawal_id`로 본다.
3. 체인 실행 문제는 `attempt_id`, `nonce_key`, `tx_hash`로 본다.
4. 운영자는 “로그가 많으냐”보다 “동일 식별자로 한 건을 재구성할 수 있느냐”를 봐야 한다.

실습 질문:

- `tx_hash`가 바뀌었는데 같은 `nonce_key`라면 replacement인가?
- `withdrawal_id`는 같은데 `attempt_id`가 늘었다면 무엇이 일어난 것인가?
- `ledger.settle` 로그가 없는데 `tracker.included`만 있다면 어느 단계가 비어 있는가?
...
### 4.3.1 관측성 대시보드에 꼭 있어야 하는 패널

강의에서 대시보드 예시를 한 장 넣을 수 있다면 아래 4개 패널이 우선순위가 높다.

1. `broadcast_to_inclusion` 지연 분포
2. `pending_age_p95`와 `replaced/dropped` 추이
3. RPC 공급자별 `error_rate`, `head_mismatch_rate`, `receipt_null_rate`
4. `RESERVE without SETTLE` 건수와 체류 시간

이 4개만 있어도 “전파 문제인지, 체인 문제인지, 원장 반영 문제인지”를 빠르게 분류할 수 있다.
...
### 5.1 현재 구현 리뷰 때 반드시 확인할 질문

강의에서는 “이상적인 신뢰성 설계”만 설명하지 말고, 현재 구현이 어디까지 와 있는지 점검하는 질문도 같이 제공하는 편이 좋다.

1. Confirmation Tracker가 `INCLUDED`까지만 반영하는가, 아니면 `SAFE/FINALIZED`까지 별도 판정하는가?
2. `SETTLE` 원장 반영이 finality 이후에만 실행되도록 보장되는가?
3. nonce 예약에 DB 유니크 제약이 있는가, 아니면 애플리케이션 메모리 락에만 의존하는가?
4. `FAILED_TIMEOUT`과 `DROPPED`를 구분하고 있는가, 아니면 둘 다 단순 실패로 처리하는가?
5. `tx_hash` 외에 `(chain, from, nonce)`를 기준으로 canonical attempt를 다시 찾을 수 있는가?

강사가 여기서 강조할 메시지:

> “신뢰성 설계의 평가는 ‘잘 되는가’가 아니라, ‘어디까지는 자동화됐고 어디부터는 아직 운영 가정인가’를 분명히 아는가’에 가깝다.”
...
- **Q: “receipt가 나왔는데 왜 ledger를 바로 확정하면 안 되나요?”**  
  **A:** receipt는 “체인에 포함되었다”는 관측 사실일 뿐이고, 내부 ledger의 settle은 “되돌릴 수 없다고 판단했다”는 회계 선언이다. 고액 출금이나 리스크가 큰 자산은 `INCLUDED` 직후가 아니라 `SAFE` 또는 `FINALIZED` 이후에 `SETTLE`해야 한다.

- **Q: “pending nonce를 읽어서 바로 쓰면 충분하지 않나요?”**  
  **A:** 그렇지 않다. pending nonce 조회는 참고값일 뿐이며, 병렬 워커 환경에서는 여전히 경쟁 상태가 발생한다. 주소별 single-writer, `UNIQUE(chain, from, nonce)`, reservation 상태 관리가 함께 있어야 한다.
...
이 장은 **본 강의의 필수 범위라기보다 심화/부록 성격**으로 두는 편이 적절하다. custody 핵심 개념을 먼저 소화한 뒤, “같은 신뢰성 원칙이 API 결제에도 어떻게 확장되는가”를 보여 주는 응용 사례로 사용하면 된다.

강의 운영 권장:

1. 기본 트랙에서는 5분 요약만 다룬다.
2. 심화 트랙 또는 Q&A 세션에서 상태머신과 식별자 설계를 자세히 확장한다.
3. 수강생이 custody의 핵심 경계와 원장 개념을 아직 소화하지 못했다면 x402 설명은 뒤로 미룬다.
...
x402는 흥미로운 확장 주제지만, 강의의 핵심 학습 목표인 `Withdrawal/TxAttempt`, ledger, finality, approval/signer 분리보다 앞에 오면 오히려 메시지가 흐려질 수 있다. 따라서 다음 순서를 권장한다.

1. 수탁 지갑 기본 구조 이해
2. 출금 라이프사이클과 원장 정합성 이해
3. 운영 신뢰성 원칙 이해
4. 마지막에 x402 같은 응용 계층으로 확장
