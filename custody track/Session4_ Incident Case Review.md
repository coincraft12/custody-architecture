...
### B-1. 화이트리스트와 정책 변경을 상태머신으로 설명하기

Ronin 사례를 강의할 때 “권한이 남아 있었다”는 서술만으로는 추상적이다. 수강생이 실제 설계를 떠올릴 수 있도록, 화이트리스트와 정책 변경을 각각 상태머신으로 보여 주는 편이 효과적이다.

화이트리스트 상태머신 예시:

`REGISTERED -> APPROVED -> HOLDING(기본 24~48h) -> ACTIVE -> EXPIRED/REVOKED`

각 상태의 의미:

- `REGISTERED`: 주소 등록 요청이 생성되었지만 아직 승인되지 않음
- `APPROVED`: 승인 quorum은 충족했지만 출금에 바로 사용되지는 않음
- `HOLDING`: 냉각 기간(time delay) 동안 활성화 보류
- `ACTIVE`: 실제 출금 정책에 사용 가능
- `EXPIRED/REVOKED`: 만료 또는 수동 회수로 더 이상 사용 불가

정책 변경 상태머신 예시:

`PROPOSED -> QUORUM_APPROVED -> DELAYED -> APPLIED -> ROLLED_BACK`

강의 포인트:

1. 화이트리스트와 정책 변경은 “출금 승인”과 동일한 워크플로우로 다루면 안 된다.
2. `APPROVED`와 `ACTIVE`를 분리해야 잔존 권한 사고를 줄일 수 있다.
3. 변경 이벤트는 모두 감사 로그와 correlation id로 남겨야 한다.
4. 긴급 우회(emergency bypass)가 있다면 만료시간과 사후 검토를 반드시 붙여야 한다.

강사가 던질 질문:

- “이 주소는 승인되었지만 아직 냉각 기간 중인가?”
- “정책 변경은 승인 즉시 적용되는가, 아니면 지연 적용되는가?”
- “예외적으로 풀어 준 권한은 자동으로 회수되는가?”
...
### 5.1 상태 머신에 정책 변경 이벤트를 섞지 말아야 하는 이유

출금 상태 머신과 화이트리스트/정책 변경 상태 머신을 한 표에 섞어 버리면, 수강생은 “정책 변경도 출금 한 단계”처럼 받아들이기 쉽다. 하지만 실제로는 다음과 같이 분리해야 한다.

- 출금 상태 머신: 자금 이동의 업무 흐름
- 화이트리스트 상태 머신: 대상 주소 허용의 생명주기
- 정책 변경 상태 머신: 한도/규칙/권한 구조의 생명주기

이 셋을 분리해야 다음 질문에 답할 수 있다.

1. 출금은 정상 승인됐지만 대상 주소가 아직 `HOLDING`이면 어떻게 할 것인가?
2. 정책 변경이 `QUORUM_APPROVED` 상태지만 아직 `DELAYED`라면 어느 규칙을 적용할 것인가?
3. 이미 브로드캐스트된 출금에 대해 이후 정책 변경이 적용될 수 있는가?
