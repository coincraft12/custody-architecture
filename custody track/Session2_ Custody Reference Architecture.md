...
### 3.2.1 Signer 입력 계약을 어디까지 구체화해야 하는가

강의에서 “Signer는 서명만 한다”고 말하는 것만으로는 부족하다. 실무 설계에서는 Signer가 **무엇을 입력으로 받고 무엇을 거부해야 하는지**가 더 중요하다. 수강생에게는 아래처럼 “Signer Input Contract”를 별도 산출물로 상정하게 하는 편이 좋다.

필수 입력 필드 예시:

| 필드 | 의미 | Signer가 확인해야 할 것 |
|---|---|---|
| `request_id` | 서명 요청 고유 ID | 재사용된 요청 ID가 아닌가 |
| `withdrawal_id` | 업무 단위 식별자 | 승인된 출금과 연결되는가 |
| `attempt_id` | 체인 시도 식별자 | 동일 attempt 재서명이 아닌가 |
| `chain_id` | 체인 식별자 | 허용된 체인인가, replay 위험은 없는가 |
| `verifying_contract` | 검증 대상 계약 | 정책상 허용된 계약인가 |
| `from`, `to`, `amount` | 자금 이동 핵심 값 | 승인된 payload와 바이트 단위로 일치하는가 |
| `nonce` | 재생 방지 값 | 이미 사용된 nonce가 아닌가 |
| `deadline` 또는 `valid_before` | 유효시간 | 만료되지 않았는가 |
| `policy_decision_id` | 정책 엔진 판정 근거 | 승인된 정책 결정과 연결되는가 |
| `approval_bundle_id` | 승인 묶음 식별자 | 요구 quorum을 충족했는가 |
| `signing_scope` | 허용된 메서드/자산/한도 | 광범위 권한 위임이 아닌가 |

Signer가 거부해야 하는 대표 사례:

1. 승인된 `to` 주소와 실제 서명 payload의 `to` 주소가 다를 때
2. `chain_id` 또는 `verifying_contract`가 정책 범위를 벗어날 때
3. 만료된 승인 토큰이나 이미 사용된 `request_id`가 재전송될 때
4. “이번 한 건” 승인인데 무제한 allowance나 임의 calldata가 들어올 때

강의용 권장 멘트:

> “Signer는 정책을 결정하지 않지만, 정책이 반영된 구조화 입력을 검증하는 마지막 방어선이어야 한다.”
...
### 보강 메모: 목표 아키텍처와 현재 실습 구현을 구분해서 설명하기

수강생은 아키텍처 다이어그램을 보면 종종 “이 저장소가 이미 이 수준으로 다 구현돼 있다”고 받아들인다. 강의에서는 아래를 명시적으로 구분하는 편이 좋다.

| 항목 | 목표 아키텍처 | 현재 실습 구현에서 흔히 생략/단순화되는 부분 |
|---|---|---|
| Confirmation Tracker | included/safe/finalized를 구분해 완료 선언 | receipt 발견 시점의 included 중심 추적 |
| Ledger | reserve 후 finality 충족 시 settle | reserve만 먼저 보여 주고 settle 자동화는 축약될 수 있음 |
| Nonce 관리 | reservation 테이블 + DB 유니크 제약 + address별 single-writer | 메모리 락 또는 단순 pending nonce 조회에 의존하기 쉬움 |
| 예외 처리 | 6종 예외를 별도 상태/런북으로 운영 | timeout/failed 중심으로 단순화되기 쉬움 |

강의용 권장 멘트:

> “이 다이어그램은 운영 완성형 기준이고, 실습 코드는 그중 핵심 경계와 전이 원리를 먼저 보여 주는 최소 구현이다.”
