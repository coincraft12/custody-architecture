...
### 7.1 Degrade mode를 어떻게 설명할 것인가

강의안에서 “degrade mode로 전환한다”는 표현만 있으면 수강생은 구체적으로 무엇이 달라지는지 감을 못 잡는다. 아래처럼 단계별 운용 모드를 제시하는 편이 좋다.

| 모드 | 진입 조건 예시 | 시스템 동작 | 운영자 액션 |
|---|---|---|---|
| `NORMAL` | RPC quorum 안정, mismatch 낮음 | 자동 브로드캐스트/자동 추적 | 정기 모니터링 |
| `DEGRADED_READ` | head mismatch 증가, 일부 receipt 불일치 | 신규 출금은 허용하되 완료 선언을 보수적으로 지연 | 특정 RPC 공급자 상태 점검 |
| `DEGRADED_WRITE` | 다수 RPC 오류, 브로드캐스트 성공 신뢰도 저하 | 고위험 출금 중단, 저위험만 제한적으로 허용 | 백업 RPC 또는 자체 노드 전환 |
| `MANUAL_APPROVAL_ONLY` | quorum 붕괴, reorg 급증, 공급자 광범위 장애 | 신규 자동 출금 중지, 수동 승인만 허용 | 상황판 공유, 정책 임시 강화 |
| `STOP_THE_LINE` | 체인/노드 신뢰 붕괴, 대규모 이상 징후 | 모든 신규 브로드캐스트 중지 | 운영 사고 프로세스 발동 |

강의 포인트:

1. degrade mode는 “서비스 다운”이 아니라, **위험을 통제된 방식으로 축소하는 운영 상태**다.
2. `read path`와 `write path`를 분리해서 생각해야 한다. 읽기 불확실성과 쓰기 불확실성의 대응은 다르다.
3. 모드 전환 기준은 사람 감이 아니라 `head_mismatch_rate`, `receipt_null_rate`, `rpc_error_rate` 같은 지표로 정의해야 한다.

강사가 던질 질문:

- “현재는 읽기만 불안정한가, 아니면 쓰기도 신뢰할 수 없는가?”
- “고위험 출금만 멈출 것인가, 전체를 멈출 것인가?”
- “정상 모드로 복귀하는 조건은 무엇인가?”
...
## 9.1 심화 부록: 체인별 확장 사례를 어디까지 다룰 것인가

이 세션의 본류는 “체인 차이를 안전하게 노출하는 계약”을 이해하는 것이다. 따라서 아래 사례들은 **심화 부록**으로 짧게 다루고, 본 강의의 핵심 메시지를 흐리지 않는 선에서 소개하는 편이 좋다.

| 체인/계열 | 한 줄 핵심 | 강의에서의 위치 |
|---|---|---|
| Bitcoin/UTXO | nonce 대신 입력 UTXO 선택이 충돌과 수수료를 좌우 | 선택 심화 |
| Cosmos | `account_number + sequence`가 replay 방지와 순차성의 핵심 | 선택 심화 |
| Solana | recent blockhash와 durable nonce가 EVM nonce와 다름 | 선택 심화 |
| L2/rollup | L1 finality와 L2 실행 finality를 분리해서 봐야 함 | 선택 심화 |

강의 운영 팁:

1. EVM 중심 청중이면 본문은 EVM으로 끝내고, 나머지는 “확장 사례 1장”으로 압축한다.
2. 체인별 API 표면 차이보다 “무엇이 멱등성 키이고 무엇이 완료 기준인가”에 초점을 맞춘다.
3. 수강생이 멀티체인 자체보다 custody 설계를 배우는 자리라면, 체인별 디테일을 너무 길게 끌지 않는다.
