...
## 7. 원장 기록: 2단계 패턴 (RESERVE → SETTLE)

### 7.1 왜 두 번 기록하는가

출금 한 건에 대해 내부 원장(ledger) 엔트리는 정확히 **두 개** 생성됩니다. 이는 버그가 아니라 브로드캐스트 시점과 온체인 확정 시점 사이의 **시간 간격**을 안전하게 다루기 위한 설계입니다.

| 엔트리 타입 | 발생 시점 | Withdrawal 상태 | 목적 |
|------------|----------|----------------|------|
| **RESERVE** | 승인 직후, 브로드캐스트 전 | W3_APPROVED | 해당 자금을 "출금 예약"으로 묶어 다른 곳에 쓰이지 않도록 차단 |
| **SETTLE** | 온체인 최종 확정 후 | W8_SAFE_FINALIZED → W9_LEDGER_POSTED → W10_COMPLETED | 실제로 체인에서 자금이 나갔음을 확정하고 잔액을 최종 반영 |

### 7.2 RESERVE의 역할

W3_APPROVED 시점에 RESERVE 엔트리를 기록하는 이유는 **이중 출금 방지**입니다. 브로드캐스트 이후 온체인 확정까지는 수십 초~수 분의 간격이 존재합니다. 이 구간 동안 RESERVE 없이 잔액만 보고 출금 가능 여부를 판단하면, 동일 자금에 대해 새로운 출금 요청이 승인될 수 있습니다.

```
브로드캐스트 → [블록 채굴 대기 구간] → 온체인 포함 → [Confirmation 대기] → Finalized
    ↑                                                                              ↑
 RESERVE 기록                                                                  SETTLE 기록
 (자금 잠금)                                                                  (잔액 확정)
```

RESERVE가 기록된 이후 잔액 계산 시 예약된 자금을 차감하면, 이 구간 동안 동일 자금에 대한 신규 출금 요청을 자연스럽게 차단할 수 있습니다.
...
RESERVE와 SETTLE은 기존 레코드를 수정하지 않고 **새 행을 추가(append-only)** 하는 방식으로 기록됩니다. 이는 감사(audit) 추적성을 보장하고, 장애 복구 시 어느 단계까지 처리됐는지 원장 이벤트 순서만으로 재구성할 수 있게 합니다. 모니터링 관점에서는 RESERVE는 있지만 SETTLE이 없는 건이 장기간 지속되면 온체인 확정 지연이나 재시도 문제를 나타내는 신호로 활용할 수 있습니다.

## 8. 운영 로직: 프로처럼 보이는 포인트
...
7. **재무/회계 처리**: W8_SAFE_FINALIZED 이후 SETTLE 원장 엔트리를 기록하고 W9_LEDGER_POSTED → W10_COMPLETED로 전환합니다. 브로드캐스트 시점(W3)에 RESERVE, 확정 시점(W8)에 SETTLE을 각각 append-only로 기록하는 2단계 패턴으로 이중 출금을 방지하고 감사 추적성을 확보합니다(Section 7 참고).
