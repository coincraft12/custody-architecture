...
## 6) 실습 2 — Retry / Replace / Included 수렴 (실체인)
...
- attempt 3개
- 직전 canonical attempt가 `REPLACED`, `canonical=false`
- 최신 attempt `canonical=true`
...
- 트랜잭션 receipt가 확인되면 Withdrawal 상태가 `W7_INCLUDED`로 전이
- canonical attempt 상태가 `INCLUDED` 후 성공 receipt 시 `SUCCESS`로 전이
